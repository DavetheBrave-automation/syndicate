...
def open_position(ticker: str, contracts: int, ask_cents: float,
                  asset_class: str, expires_at: int = None,
                  side: str = "yes") -> bool:
    """Buy YES or NO at ask. Returns True on success."""
    if contracts <= 0:
        return False
    ...
    if TRADING_MODE == "LIVE":
        order_kwargs = {"count": contracts, "order_type": "limit"}
        if side == "yes":
            order_kwargs["yes_price"] = int(ask_cents)
        else:
            order_kwargs["no_price"] = int(ask_cents)
        res = kalshi.place_order(ticker, side=side, action="buy", **order_kwargs)
        if not res:
            db.log("ERROR", f"LIVE order failed — {ticker} {side.upper()}")
            return False
    ...
    db.log("ENTRY", f"BUY {contracts} {ticker} {side.upper()} @ {ask_cents:.1f}¢",
...
def close_position(ticker: str, exit_cents: float, reason: str) -> bool:
    ...
    if TRADING_MODE == "LIVE":
        ...
        kalshi.place_order(ticker, side=exit_side, action="sell",
